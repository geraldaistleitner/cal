...
class SearchViews extends \TYPO3\CMS\Cal\View\ListView
{
    ...
    public function drawSearchAllResult(&$master_array, $starttime, $endtime, $searchword, $locationIds = '', $organizerIds = '')
    {
        if ($this->conf ['activateFluid'] == 1) {
            ...
            return $this->drawSearchEventResult($master_array['phpicalendar_event'], $starttime, $endtime, $searchword, $locationIds, $organizerIds);
        }
    ...
    public function drawSearchEventResult(&$master_array, $starttime, $endtime, $searchword, $locationIds = '', $organizerIds = '')
    {
        ...
        $this->_init($master_array);
        if ($this->conf ['activateFluid'] == 1) {
            return $this->fluidSearch($searchword, $starttime, $endtime, $organizerIds);
        }

        return $this->drawList($master_array, '', $starttime, $endtime);
    }

    private function fluidSearch($query, $starttime, $endtime, $organizerIds)
    {
        $object = new \stdClass();
        $object->calendar = $this->getCalendars();
        $object->searchActionURL = $this->local_cObj->cObjGetSingle($this->conf ['view.'] ['search.'] ['searchLinkUrl'], $this->conf ['view.'] ['search.'] ['searchLinkUrl.']);
        $object->view = $this->conf ['view'];
        $outputFormat = \TYPO3\CMS\Cal\Utility\Functions::getFormatStringFromConf($this->conf);
        $object->starttime = $starttime->format($outputFormat);
        $object->endtime = $endtime->format($outputFormat);
        $object->categories = $this->getCategories();
        $object->locations = $this->getLocations();
        $object->query = $query;
        $object->organizers = $this->getOrganizers($organizerIds);
        return $this->renderWithFluid($object);
    }

    private function getCalendars()
    {
        $catArrayArray = $this->modelObj->findAllCalendar('', $this->conf ['pidList']);
        $calendars = array();
        $rememberUid = Array();
        $ids = Array();

        $calendars[] = array(id => 0, selected => 0, title => $this->controller->pi_getLL('l_all'));
        if ($this->controller->piVars ['submit'] && $this->controller->piVars ['calendar']) {
            $ids = $this->controller->piVars ['calendar'];
        }
        foreach ($catArrayArray as $categoryArrayFromService) {
            foreach ($categoryArrayFromService as $category) {
                $uid = $category->getUid();
                if (!in_array($uid, $rememberUid)) {
                    if (in_array($uid, $ids)) {
                        $calendars [] = array(id => $uid, selected => 1, title => $category->getTitle());
                    } else {
                        $calendars [] = array(id => $uid, selected => 0, title => $category->getTitle());
                    }
                    $rememberUid [] = $uid;
                }
            }
        }
        return array(calendar => $calendars, selected => $ids);
    }
    ...
    private function getCategories()
    {
        ...
        $categories[] = array(id => 0, selected => 0, title => $this->controller->pi_getLL('l_all'));
        if ($this->controller->piVars ['submit'] && $this->controller->piVars ['category']) {
            $ids = $this->controller->piVars ['category'];
        }
        foreach ($catArrayArray as $categoryArrayFromService) {
            foreach ($categoryArrayFromService [0] [0] as $category) {
                ...
                if (!in_array($uid, $rememberUid)) {
                    if (in_array($uid, $ids)) {
                        $categories [] = array(id => $uid, selected => 1, title => $category->getTitle());
                    } else {
                        $categories [] = array(id => $uid, selected => 0, title => $category->getTitle());
                    }
                    $rememberUid [] = $uid;
                }
            }
        }
        return array(categories => $categories, selected => $ids);
    }
    ...
    private function getLocations()
    {
        ...
        if (is_array($locationArray)) {
            foreach ($locationArray as $location) {
                if (in_array($location->getUid(), $locationIdArray)) {
                    $locations[] = array(id => $location->getUid(), checked => 1, title => $location->getName());
                } else {
                    $locations[] = array(id => $location->getUid(), checked => 0, title => $location->getName());
                }
            }
        }
        return array(locations => $locations, selected => $locationIdArray);
    }

    private function getOrganizers($organizerIds)
    {
        $organizerArray = $this->modelObj->findAllOrganizer($this->extConf ['useOrganizerStructure'] ? $this->extConf ['useOrganizerStructure'] : 'tx_cal_organizer', $this->conf ['pidList']);
        $organizers = array();
        $organizers[] = array(id => 0, selected => 0, title => $this->controller->pi_getLL('l_all'));
        $organizerIdArray = Array();
        ...
        if (is_array($organizerArray)) {
            foreach ($organizerArray as $organizer) {
                if (in_array($organizer->getUid(), $organizerIdArray)) {
                    $organizers[] = array(id => $organizer->getUid(), selected => 1, title => $organizer->getName());
                } else {
                    $organizers[] = array(id => $organizer->getUid(), selected => 0, title => $organizer->getName());
                }
            }
        }
        return array(organizers => $organizers, selected => $organizerIdArray);
    }
